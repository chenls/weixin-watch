...
final class EANManufacturerOrgSupport {
    ...
    EANManufacturerOrgSupport() {
        this.countryIdentifiers = new ArrayList<String>();
    }
    ...
    private void initIfNeeded() {
        synchronized (this) {
            block6: {
                boolean bl2 = this.ranges.isEmpty();
                if (bl2) break block6;
                return;
            }
     ...
     */
    String lookupCountryIdentifier(String object) {
        this.initIfNeeded();
        int n2 = Integer.parseInt(((String)object).substring(0, 3));
        int n3 = this.ranges.size();
        int n4 = 0;
        Object object2;
        while (n4 < n3 && n2 >= (object2 = (object = (Object)this.ranges.get(n4))[0])) {
            if (((Object)object).length != 1) {
                object2 = object[1];
            }
            if (n2 <= object2) {
                return this.countryIdentifiers.get(n4);
            }
            ++n4;
        }
        return null;
