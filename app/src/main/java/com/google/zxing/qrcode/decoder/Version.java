...
public final class Version {
    ...
    private Version(int n2, int[] objectArray, ECBlocks ... object) {
        int n3 = 0;
        this.versionNumber = n2;
        this.alignmentPatternCenters = objectArray;
        this.ecBlocks = object;
        int n4 = 0;
        int n5 = object[0].getECCodewordsPerBlock();
        objectArray = object[0].getECBlocks();
        int n6 = objectArray.length;
        n2 = n3;
        n3 = n4;
        while (n2 < n6) {
            object = objectArray[n2];
            n3 += ((ECB)object).getCount() * (((ECB)object).getDataCodewords() + n5);
            ++n2;
        }
    ...
    public static Version getProvisionalVersionForDimension(int n2) throws FormatException {
        ...
        try {
            Version version = Version.getVersionForNumber((n2 - 17) / 4);
            return version;
        }
    ...
    BitMatrix buildFunctionPattern() {
        ...
        bitMatrix.setRegion(n2 - 8, 0, 8, 9);
        bitMatrix.setRegion(0, n2 - 8, 9, 8);
        int n3 = this.alignmentPatternCenters.length;
        int n4 = 0;
        while (true) {
            int n5;
            if (n4 < n3) {
                n5 = this.alignmentPatternCenters[n4];
            } else {
                bitMatrix.setRegion(6, 9, 1, n2 - 17);
                bitMatrix.setRegion(9, 6, n2 - 17, 1);
                if (this.versionNumber > 6) {
                    bitMatrix.setRegion(n2 - 11, 0, 3, 6);
                    bitMatrix.setRegion(0, n2 - 11, 6, 3);
                }
                return bitMatrix;
            }
            for (int i2 = 0; i2 < n3; ++i2) {
                if (n4 == 0 && (i2 == 0 || i2 == n3 - 1) || n4 == n3 - 1 && i2 == 0) continue;
                bitMatrix.setRegion(this.alignmentPatternCenters[i2] - 2, n5 - 2, 5, 5);
            }
            ++n4;
        }
    }
    ...
    public static final class ECBlocks {
        ...
        public int getNumBlocks() {
            int n2 = 0;
            ECB[] eCBArray = this.ecBlocks;
            int n3 = eCBArray.length;
            for (int i2 = 0; i2 < n3; ++i2) {
                n2 += eCBArray[i2].getCount();
            }
            return n2;
        }
