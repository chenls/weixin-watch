...
class ChildHelper {
    ...
    static class Bucket {
        ...
        boolean get(int n2) {
            ...
            return (this.mData & 1L << n2) != 0L;
        }

        /*
         * Enabled aggressive block sorting
         */
        void insert(int n2, boolean bl2) {
            if (n2 >= 64) {
                this.ensureNext();
                this.next.insert(n2 - 64, bl2);
                return;
            } else {
                boolean bl3 = (this.mData & Long.MIN_VALUE) != 0L;
                long l2 = (1L << n2) - 1L;
                this.mData = this.mData & l2 | (this.mData & (0xFFFFFFFFFFFFFFFFL ^ l2)) << 1;
                if (bl2) {
                    this.set(n2);
                } else {
                    this.clear(n2);
                }
                if (!bl3 && this.next == null) return;
                this.ensureNext();
                this.next.insert(0, bl3);
                return;
            }
        }
        ...
        boolean remove(int n2) {
            ...
            long l2 = 1L << n2;
            boolean bl2 = (this.mData & l2) != 0L;
            this.mData &= 0xFFFFFFFFFFFFFFFFL ^ l2;
            this.mData = this.mData & --l2 | Long.rotateRight(this.mData & (0xFFFFFFFFFFFFFFFFL ^ l2), 1);
            boolean bl3 = bl2;
            if (this.next == null) return bl3;
            if (this.next.get(0)) {
                this.set(63);
            }
            this.next.remove(0);
