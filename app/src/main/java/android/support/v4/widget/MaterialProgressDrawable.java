...
implements Animatable {
    ...
    private int evaluateColorChange(float f2, int n2, int n3) {
        ...
        int n6 = n4 >> 8 & 0xFF;
        n3 = n3;
        return (int)((float)((n3 >> 24 & 0xFF) - n2) * f2) + n2 << 24 | (int)((float)((n3 >> 16 & 0xFF) - n5) * f2) + n5 << 16 | (int)((float)((n3 >> 8 & 0xFF) - n6) * f2) + n6 << 8 | (int)((float)((n3 & 0xFF) - (n4 &= 0xFF)) * f2) + n4;
    }
    ...
    private void setupAnimators() {
        ...
        Animation animation = new Animation(){
            ...
            public void applyTransformation(float f2, Transformation transformation) {
                ...
                if (f2 <= 0.5f) {
                    float f7 = f2 / 0.5f;
                    f7 = MATERIAL_INTERPOLATOR.getInterpolation(f7);
                    ring.setStartTrim(f5 + (0.8f - f3) * f7);
                }
                if (f2 > 0.5f) {
                    f5 = (f2 - 0.5f) / 0.5f;
                    f5 = MATERIAL_INTERPOLATOR.getInterpolation(f5);
    ...
    float getMinProgressArc(Ring ring) {
        return (float)Math.toRadians((double)ring.getStrokeWidth() / (Math.PI * 2 * ring.getCenterRadius()));
    }
