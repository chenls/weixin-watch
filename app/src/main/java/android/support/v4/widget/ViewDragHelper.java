...
public class ViewDragHelper {
    ...
    private boolean checkNewEdgeDrag(float f2, float f3, int n2, int n3) {
        block5: {
            block4: {
                f2 = Math.abs(f2);
                f3 = Math.abs(f3);
                if ((this.mInitialEdgesTouched[n2] & n3) != n3 || (this.mTrackingEdges & n3) == 0 || (this.mEdgeDragsLocked[n2] & n3) == n3 || (this.mEdgeDragsInProgress[n2] & n3) == n3 || f2 <= (float)this.mTouchSlop && f3 <= (float)this.mTouchSlop) break block4;
                if (f2 < 0.5f * f3 && this.mCallback.onEdgeLock(n3)) {
                    int[] nArray = this.mEdgeDragsLocked;
                    nArray[n2] = nArray[n2] | n3;
                    return false;
                }
                if ((this.mEdgeDragsInProgress[n2] & n3) == 0 && f2 > (float)this.mTouchSlop) break block5;
            }
            return false;
        }
        return true;
    ...
    private boolean isValidPointerForActionMove(int n2) {
        if (!this.isPointerDown(n2)) {
            Log.e((String)TAG, (String)("Ignoring pointerId=" + n2 + " because ACTION_DOWN was not received " + "for this pointer before ACTION_MOVE. It likely happened because " + " ViewDragHelper did not receive all the events in the event stream."));
            return false;
        }
        return true;
    }
    ...
    public boolean isEdgeTouched(int n2, int n3) {
        return this.isPointerDown(n3) && (this.mInitialEdgesTouched[n3] & n2) != 0;
    }
