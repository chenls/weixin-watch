...
class FocusStrategy {
    ...
    private static boolean beamBeats(int n2, @NonNull Rect rect, @NonNull Rect rect2, @NonNull Rect rect3) {
        boolean bl2 = true;
        boolean bl3 = FocusStrategy.beamsOverlap(n2, rect, rect2);
        if (FocusStrategy.beamsOverlap(n2, rect, rect3)) return false;
        if (!bl3) {
            return false;
        }
        bl3 = bl2;
        if (!FocusStrategy.isToDirectionOf(n2, rect, rect3)) return bl3;
        bl3 = bl2;
        if (n2 == 17) return bl3;
        bl3 = bl2;
        if (n2 == 66) return bl3;
        bl3 = bl2;
        if (FocusStrategy.majorAxisDistance(n2, rect, rect2) < FocusStrategy.majorAxisDistanceToFarEdge(n2, rect, rect3)) return bl3;
        return false;
    }
    ...
    public static <L, T> T findNextFocusInAbsoluteDirection(@NonNull L l2, @NonNull CollectionAdapter<L, T> collectionAdapter, @NonNull BoundsAdapter<T> boundsAdapter, @Nullable T t2, @NonNull Rect rect, int n2) {
        ...
        for (int i2 = 0; i2 < n3; ++i2) {
            L l3 = collectionAdapter.get(l2, i2);
            if (l3 == t2) continue;
            boundsAdapter.obtainBounds(l3, rect3);
            if (!FocusStrategy.isBetterCandidate(n2, rect, rect3, rect2)) continue;
            rect2.set(rect3);
            t3 = (T)l3;
        }
        return t3;
    ...
    public static <L, T> T findNextFocusInRelativeDirection(@NonNull L l2, @NonNull CollectionAdapter<L, T> collectionAdapter, @NonNull BoundsAdapter<T> boundsAdapter, @Nullable T t2, int n2, boolean bl2, boolean bl3) {
        int n3 = collectionAdapter.size(l2);
        ArrayList<T> arrayList = new ArrayList<T>(n3);
        for (int i2 = 0; i2 < n3; ++i2) {
            arrayList.add(collectionAdapter.get(l2, i2));
        }
        Collections.sort(arrayList, new SequentialComparator<T>(bl2, boundsAdapter));
        switch (n2) {
            default: {
                throw new IllegalArgumentException("direction must be one of {FOCUS_FORWARD, FOCUS_BACKWARD}.");
            }
            case 2: {
                return FocusStrategy.getNextFocusable(t2, arrayList, bl3);
            }
            case 1: 
        }
        return FocusStrategy.getPreviousFocusable(t2, arrayList, bl3);
    }

    /*
     * Enabled aggressive block sorting
     */
    private static <T> T getNextFocusable(T t2, ArrayList<T> arrayList, boolean bl2) {
     ...
     * Enabled aggressive block sorting
     */
    private static <T> T getPreviousFocusable(T t2, ArrayList<T> arrayList, boolean bl2) {
        int n2 = arrayList.size();
    ...
    private static boolean isBetterCandidate(int n2, @NonNull Rect rect, @NonNull Rect rect2, @NonNull Rect rect3) {
        ...
        boolean bl3 = bl2;
        if (!FocusStrategy.isCandidate(rect, rect3, n2)) return bl3;
        bl3 = bl2;
        if (FocusStrategy.beamBeats(n2, rect, rect2, rect3)) return bl3;
        if (FocusStrategy.beamBeats(n2, rect, rect3, rect2)) {
            return false;
        }
        bl3 = bl2;
        if (FocusStrategy.getWeightedDistanceFor(FocusStrategy.majorAxisDistance(n2, rect, rect2), FocusStrategy.minorAxisDistance(n2, rect, rect2)) < FocusStrategy.getWeightedDistanceFor(FocusStrategy.majorAxisDistance(n2, rect, rect3), FocusStrategy.minorAxisDistance(n2, rect, rect3))) return bl3;
        return false;
    }
    ...
    implements Comparator<T> {
        ...
        SequentialComparator(boolean bl2, BoundsAdapter<T> boundsAdapter) {
